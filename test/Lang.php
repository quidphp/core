...
class Lang extends Base\Test
{
    ...
    final public static function trigger(array $data):bool
    {
        ...
        assert(Base\Datetime::getFormatReplace('y-@m@-d','fr')['replace']['@'][1] === 'Janvier');
        assert(Base\Datetime::parseFormat([0,'lang'=>'fr'])['format'] === 'j %n% Y');
        assert(Base\Datetime::format(['j %m% Y','lang'=>'fr'],$timestamp) === '1 décembre 2017');
        assert(Base\Datetime::format(['format'=>0,'lang'=>'fr'],$timestamp) === '1 décembre 2017');
        assert(Base\Datetime::format(['format'=>true,'lang'=>'fr'],$timestamp) === '1 décembre 2017');
        assert(Base\Datetime::format(['format'=>1,'lang'=>'fr'],$timestamp) === '1 décembre 2017 09:40:42');
        assert(Base\Datetime::format(['format'=>2,'lang'=>'fr'],$timestamp) === 'Décembre 2017');
        assert(Base\Datetime::format(['format'=>3,'lang'=>'fr'],$timestamp) === '01-12-2017');
        assert(Base\Datetime::format(['format'=>4,'lang'=>'fr'],$timestamp) === '01-12-2017 09:40:42');
        assert(Base\Datetime::parse(['j %m% Y','lang'=>'fr'],'1 décembre 2017') === ['year'=>2017,'month'=>12,'day'=>1]);
        assert(Base\Datetime::parse([0,'lang'=>'fr'],'12 décembre 2017') === ['year'=>2017,'month'=>12,'day'=>12]);
        assert(Base\Datetime::parse([2,'lang'=>'fr'],'Décembre 2017') === ['year'=>2017,'month'=>12]);
        assert(Base\Datetime::time('20 décembre 2017',['format'=>'j %m% Y','lang'=>'fr']) === 1513746000);
        assert(Base\Datetime::str(6,['year'=>12,'month'=>3,'day'=>14,'hour'=>10],'fr') === '12 années 3 mois 14 jours et 10 heures');
        assert('2 000.00' === Base\Num::format('2000','fr'));
        assert(Base\Num::getFormat('fr',['decimal'=>3]) === ['decimal'=>3,'separator'=>'.','thousand'=>' ']);
        assert('2 000.00 $' === Base\Num::moneyFormat(2000,'fr'));
        assert(Base\Num::getMoneyFormat('fr',['decimal'=>3]) === ['decimal'=>3,'separator'=>'.','thousand'=>' ','output'=>'%v% $']);
        assert(Base\Num::getSizeFormat('fr',['text'=>[1=>'James']])['text'][0] === 'Octet');
        assert(count(Base\Error::getCodes('fr')) === 16);
        assert(count(Base\Datetime::getMonths('fr')) === 12);
        assert(count(Base\Datetime::getStr('fr')) === 7);
        $lang->unsetInst();
        $bootLang->setInst();
